$(document).ready(function () {
     ...
     * MANAGEMENT USERS
     *****************************************************************/
    $(document).on('click', '.upgradeStaff, .downgradeCustomer', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    // cập nhật role hiển thị ngay
                    if (response.message.includes('nhân viên')) {
                        button.closest('.profile_view').find('.brief i').text('STAFF');

                        // đổi nút thành downgrade
                        button.removeClass('upgradeStaff btn-primary')
                            .addClass('downgradeCustomer btn-info')
                            .html('<i class="fa fa-user"></i> Khách hàng');
                    } else if (response.message.includes('khách hàng')) {
                        button.closest('.profile_view').find('.brief i').text('CUSTOMER');

                        // đổi nút thành upgrade
                        button.removeClass('downgradeCustomer btn-info')
                            .addClass('upgradeStaff btn-primary')
                            .html('<i class="fa fa-user"></i> Nhân viên');
                    }
                } else {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    if (status === "banned") {
                        // đổi nút thành "Bỏ chặn"
                        button.removeClass("btn-warning")
                            .addClass("btn-success")
                            .text("Bỏ chặn")
                            .data("status", "active");
                        $('#status-' + userId).text("banned");
                        ...
                        // xóa hẳn khỏi DOM
                        button.closest('.profile_details').remove();
                        toastr.info("Người dùng đã bị xóa khỏi hệ thống");
                    }
                } else {
                    toastr.error(response.message);
                }
     ...
     * MANAGEMENT CATEGORY
     *****************************************************************/
    $(document).on("change", ".category-image, #category-image", function () {
        let file = this.files[0];

        // Nếu là input có data-id (update nhiều category)
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                if (categoryId) {
                    // Trường hợp update nhiều category
                    $("#image-preview-" + categoryId).attr("src", e.target.result).show();
                } else {
                    // Trường hợp thêm mới
...
<tr id="category-row-${categoryId}">
  <td><img src="${response.data.image}" width="80" height="80"></td>
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status) {
                ...
                // Lấy URL ảnh từ accessor thay vì images[0]
